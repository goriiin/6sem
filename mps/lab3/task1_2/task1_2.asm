...
INIT:
	...
	ldi temp,(1<<PB2) ; включение подтягивающего 
	out PORTB,temp ; резистора входа PB7
...
PROGRAM_CALL:
    ; Проверка состояния кнопки с задержкой
    sbic PORTB, PB0
    rjmp PROGRAM_CALL ; если кнопка не нажата, продолжаем проверку

    ; Добавляем задержку для устранения дребезга
    rcall DELAY
