...
Данные блоки отвечаюь за взаимодействие с аналоговой частью схемы и визуализацию конечного результата. Узел D1 представляет собой интерфейс измерительной RC-цепи, обеспечивая подключение внешнего измеряемого резистора и принимая управляющие сигналы «Старт» и «Сброс»  для начала заряда и разряда конденсатора соответственно. Выход U узла DD1 представляет аналоговое напряжение на конденсаторе RC-цепи. АЦП D3 непрерывно преобразует это напряжение U в 14-разрядный цифровой код. На рисунке @узел-ацп приведён узел, включающий интерфейс измерительной RC‑цепи и АЦП .

#рис(image("../материалы/узел-ацп.png", height: 22%))[ Узел аналого‑цифрового преобразования ] <узел-ацп>

== Блок отображения

Счетчик D9 накапливает результат подсчета тактовых импульсов в процессе измерения. Итоговое значение с D9 по 4-разрядной шине передается непосредственно на семисегментный индикатор для отображения результата измерения. На рисунке @узел-отображения показан узел отображения результата измерения.

#рис(image("../материалы/узел-отображения.png", height: 28%))[  Узел отображения результата ] <узел-отображения>

== Блок сравнения 

Блок сравнения осуществляет операцию сравнения цифрового значения напряжения, поступающего с АЦП (DD2), с пороговым значением, хранящимся в регистре константы (D3). Компаратор D4 производит непрерывное сравнение этих двух 14-разрядных кодов. При превышении измеряемым напряжением порогового уровня, компаратор D4 формирует сигнал завершения измерения. На рисунке @узел-сравнения представлен узел формирования сигнала сравнения.

#рис(image("../материалы/узел-сравнения.png", height: 35%))[  Узел формирования сигнала сравнения ] <узел-сравнения>
...
Функциональная схема изображена на рисунке @функ.

#рис(image("../материалы/функ.png"))[  Функциональная схема ] <функ>